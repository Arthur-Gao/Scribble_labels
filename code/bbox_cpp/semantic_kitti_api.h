...
public:
    ...
    unsigned short instance_;
    VelodynePoint(): x_(0.0f), y_(0.0f), z_(0.0f), intensity_(0.0f), label_(0) {};
    VelodynePoint(float x, float y, float z, float intensity, unsigned short label, unsigned short instance): x_(x), y_(y), z_(z), intensity_(intensity), label_(label), instance_(instance) {};
    ~VelodynePoint() {};
};
    ...
    std::vector<VelodynePoint> getVelodynePoints(int timeIdx) {
        ...
        for (;;) {
            ...
            if (velodyneDataSize == 0 || labelDataSize == 0)
                break;
            VelodynePoint velodynePoint(data[0], data[1], data[2], data[3], label[0], label[1]);
