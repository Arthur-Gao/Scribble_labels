...
class LaserScanVis:
  ...
  def __init__(self, scan, scan_names, label_names, offset=0,
    ...
    # self.get_fitting_box()
    self.reset()
    self.update_scan()

  def get_fitting_box(self):
    '''
    本函数是读取scan中每个instance的三维坐标，然后通过对比计算出当前instance的x、y、z的最大值和最小值，记录在inst_box_pos中
    其中inst_box_pos[label]代表着当前instance
    inst_box_pos[label][0]代表当前instance的x最小值
    inst_box_pos[label][0]代表当前instance的x最大值
    inst_box_pos[label][0]代表当前instance的y最小值
    inst_box_pos[label][0]代表当前instance的y最大值
    inst_box_pos[label][0]代表当前instance的z最小值
    inst_box_pos[label][0]代表当前instance的z最大值
    :return:
    '''
    inst_num = np.max(self.scan.inst_label) + 1
    self.inst_box_pos = [[np.finfo(np.float32).max, np.finfo(np.float32).min,
                     np.finfo(np.float32).max, np.finfo(np.float32).min,
                     np.finfo(np.float32).max, np.finfo(np.float32).min] for _ in range(inst_num)]
    for i, label in enumerate(self.scan.inst_label):
      if label == 0:
        continue
      p = self.scan.points[i]
      if p[0] < self.inst_box_pos[label][0]:
        self.inst_box_pos[label][0] = p[0]
      if p[0] > self.inst_box_pos[label][1]:
        self.inst_box_pos[label][1] = p[0]
      if p[1] < self.inst_box_pos[label][2]:
        self.inst_box_pos[label][2] = p[1]
      if p[1] > self.inst_box_pos[label][3]:
        self.inst_box_pos[label][3] = p[1]
      if p[2] < self.inst_box_pos[label][4]:
        self.inst_box_pos[label][4] = p[2]
      if p[2] > self.inst_box_pos[label][5]:
        self.inst_box_pos[label][5] = p[2]
  ...
  def get_box(self, idx):
    '''
    本函数是根据第idx个instance的xyz最大最小值，画出box
    这里的xyz是指的画box的轨迹，可以理解为plot的语法
    第一个点为x_min,y_min,z_min，下一个点是x_max,y_min,z_min，相邻的点相连，以此类推
    :param idx:
    :return:
    '''
    x_min = self.inst_box_pos[idx][0]
    x_max = self.inst_box_pos[idx][1]
    y_min = self.inst_box_pos[idx][2]
    y_max = self.inst_box_pos[idx][3]
    z_min = self.inst_box_pos[idx][4]
    z_max = self.inst_box_pos[idx][5]
    x = np.array([x_min, x_max, x_max, x_min, x_min, x_min, x_max, x_max, x_min, x_min, x_min, x_min, x_max, x_max, x_max, x_max])
    y = np.array([y_min, y_min, y_max, y_max, y_min, y_min, y_min, y_max, y_max, y_min, y_max, y_max, y_max, y_max, y_min, y_min])
    z = np.array([z_min, z_min, z_min, z_min, z_min, z_max, z_max, z_max, z_max, z_max, z_max, z_min, z_min, z_max, z_max, z_min])
    return x,y,z
  ...
  def update_scan(self):
    ...
    if self.instances:
      ...
      # 在这里添加了画intance的fitting box代码
      ######### 添加部分开始 ########
      for i in range(len(self.inst_box_pos)):# 遍历每个instance
        if self.inst_box_pos[i][0] == np.finfo(np.float32).max: # 如果当前instance id没被更新过，则跳过
          continue
        x,y,z = self.get_box(i) # 获得当前instance的xyz轨迹
        # 用create_visual_node画图
        pos = np.c_[x, y, z]
